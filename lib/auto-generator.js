...
class AutoGenerator {
        ...
        tableNames.forEach(table => {
            ...
            if (this.options.lang === 'ts') {
                ...
                if (primaryKeys.length) {
                    str += `export type #TABLE#Pk = ${primaryKeys.map((k) => `"${this.getFieldName((0, types_1.recase)(this.options.caseProp, k))}"`).join(' | ')};\n`;
                ...
                if (creationOptionalFields.length) {
                    str += `export type #TABLE#OptionalAttributes = ${creationOptionalFields.map((k) => `"${this.getFieldName((0, types_1.recase)(this.options.caseProp, k))}"`).join(' | ')};\n`;
    ...
    addTable(table) {
        ...
        lodash_1.default.uniqBy([...fields.filter(f => this.foreignKeys[table][f]), ...fields], (f) => this.getFieldName(f)).forEach((field, index) => {
    ...
    addField(table, field) {
        ...
        const fieldName = this.getFieldName((0, types_1.recase)(this.options.caseProp, field));
    ...
    addTypeScriptFields(table, isInterface) {
        ...
        return lodash_1.default.uniqBy(fields
            ...
            .map(field => [field, this.getFieldName(this.quoteName((0, types_1.recase)(this.options.caseProp, field)))]), ([f, n]) => n).reduce((str, [field, name]) => {
