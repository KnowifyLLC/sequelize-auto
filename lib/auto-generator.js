...
class AutoGenerator {
        ...
        tableNames.forEach(table => {
            ...
            if (this.options.lang === 'ts') {
                ...
                if (primaryKeys.length) {
                    str += `export type #TABLE#Pk = ${primaryKeys.map((k) => `"${this.getFieldName((0, types_1.recase)(this.options.caseProp, k), table)}"`).join(' | ')};\n`;
                ...
                if (creationOptionalFields.length) {
                    str += `export type #TABLE#OptionalAttributes = ${creationOptionalFields.map((k) => `"${this.getFieldName((0, types_1.recase)(this.options.caseProp, k), table)}"`).join(' | ')};\n`;
    ...
    addTable(table) {
        ...
        lodash_1.default.uniqBy([...fields.filter(f => { var _a; return (_a = this.foreignKeys[table]) === null || _a === void 0 ? void 0 : _a[f]; }), ...fields], (f) => this.getFieldName(f, table)).forEach((field, index) => {
    ...
    addField(table, field) {
        ...
        const fieldName = this.getFieldName((0, types_1.recase)(this.options.caseProp, field), table);
    ...
    getTypeScriptCreationOptionalFields(table) {
        ...
        return lodash_1.default.uniq(fields.filter(filterFn));
    }
    ...
    addTypeScriptFields(table, isInterface) {
        ...
        return lodash_1.default.uniqBy(fields
            .filter(field => !this.options.skipFields || !this.options.skipFields.includes(field))
            .map(field => [field, this.getFieldName(this.quoteName((0, types_1.recase)(this.options.caseProp, field)), table)]), ([f, n]) => n).reduce((str, [field, name]) => {
